package cn.showclear.gis
{
	...
	import cn.zhrz.arcgis.EventWidthData;
...
//		private var _gisDir:String ="file:///storage/04A9-15FD/LOST.DIR/roadmap"
		...
		public var currentRow:Number;
		public var currentCol:Number;
		private function fmtURL(level:Number, row:Number, col:Number):String {
			...
			if (isOffMode) {
				...
				var imagePath:String = _gisDir + "/" + level + "/" +  col + "/" + row + ".jpg";
				//trace(imagePath);
			//	LogManager.getInstance().onLogMessage("地图加载的图片为:"+imagePath)
				this.dispatchEvent(new EventWidthData(level,row,col));
