package manager
{
	...
	public class NodeAndLineManager extends EventDispatcher
	{
...
//			nodeConfig = NodeAndLineConfigManager.getInstance().NodeAndLineConfig;	
		...
		public function addBoundary(boundArr : ArrayCollection):void{
			...
			var provinceBorderObj : Object = new Object();
			provinceBorderObj.width = PathManager.getInstance().getValue("width");
			provinceBorderObj.alpha = PathManager.getInstance().getValue("alpha");
			provinceBorderObj.color = PathManager.getInstance().getValue("color");
		 ...
		 *	添加线 
		 */
		private function addLines(layer:GraphicsLayer,pointArr : ArrayCollection,vec : Vector.<Graphic>,config : Object,message:String,index:Number=0):void{
			...
			for( var i :int = index; i < pointArr.length; i++ ){
				...
				if( i == pointArr.length-1 ){
					...
					log("这条线加载了:"+pointArr.length+"数据,消耗"+(endTime - starTime)+"毫秒");
					dispatchEvent(new Event("addLineSuccessed"));
					return;
				}
//				if( i != 0 && i%10000 == 0 ){
//					setTimeout(addLines,2000,layer,pointArr,vec,config,message,i+1);
//					trace("正在添加第："+i+"条数据");
//					return;
//				}
			}
